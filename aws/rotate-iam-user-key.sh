...
function PrintHelp() {
    ...
    echo "                    used when you download the key from IAM in the AWS console."
...
EXISTING_KEY_ID=$(awk '/.AccessKeyId./{print substr($2,2,length($2)-3)}' existing-keys)
...
NEW_AWS_ACCESS_KEY_ID=$(awk '/.AccessKeyId./{print substr($2,2,length($2)-3)}' temp-key)
...
while [ "$SUCCESS" = false ] && [ "$COUNT" -lt "$MAX_COUNT" ]; do
    sleep 3
    aws s3 ls --profile temp-role "$S3_TEST_FILE"   && RETURN_CODE=$? || RETURN_CODE=$?
